...
describe("flags.buildScript", () => {
  it("emits the read op when only read is provided", () => {
    const s = __test.buildScript({ read: true });
  ...
  it("emits both ops when both are provided", () => {
    const s = __test.buildScript({ read: false, flagged: true });
  ...
  it("emits the flagged op when only flagged is provided", () => {
    const s = __test.buildScript({ flagged: false });
